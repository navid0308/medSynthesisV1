...
parser.add_argument("--numofIters", type=int, default=10000, help="number of iterations to train for")
...
parser.add_argument("--lr_netD", type=float, default=1e-4, help="Learning Rate for discriminator. Default=5e-3")
parser.add_argument("--dropout_rate", default=0.5, type=float, help="prob to drop neurons to zero: 0.2")
parser.add_argument("--decLREvery", type=int, default=1000,
...
parser.add_argument("--weight-decay", "--wd", default=1e-10, type=float, help="weight decay, Default: 1e-4")
...
parser.add_argument("--prefixPredictedFN", default="T1T2-PFN", type=str,
                    help="prefix of the to-be-saved predicted filename")
parser.add_argument("--test_input_file_name", default="IXI450-Guys-1093-T1.nii.gz", type=str,
                    help="the input file name for testing subject")
parser.add_argument("--test_gt_file_name", default="IXI450-Guys-1093-T2.nii.gz", type=str,
...
def main():
    print(opt)
    plt.figure(1)
    plt.plot(0, 0, 'g', label='Generator')
    plt.plot(0, 0, 'r', label='Discriminator')
    ...
    for iter in range(opt.start_epoch, opt.numofIters + 1):
        # print('iter %d'%iter)
        if iter % 500 == 0:
            inputs = np.random.normal(scale=1.0, size=(opt.batchSize, 5, 64, 64))
            labels = np.random.normal(scale=1.0, size=(opt.batchSize, 64, 64))
            exinputs = inputs
        else:
            if opt.isMultiSource:
        ...
        if inputs.shape[0] < opt.batchSize:
            continue
            #avoid mini-batch loss spikes
        ...
        residual_source = inputs[:, mid_slice, ...]  # WHY
        # inputs = inputs.cuda()
        ...
        for i in range(inputs.shape[0]):
            img1 = np.array(inputs[i, 0,...])
            img2 = np.array(labels[i, ...])
        ...
        if opt.isAdLoss:
            ...
            out = outputG.data.cpu().numpy().squeeze()
            import cv2
            img1 = np.array(inputs[0, 0, ...])
            img2 = np.array(labels[0, ...])
            img3 = np.array(out[0, ...])
            img = np.concatenate((img1, img3, img2), axis=1)
            cv2.imshow('baal', img)
            cv2.waitKey(1)
        ...
        # print statistics
        running_loss = running_loss + lossG_G.item()

        if iter % 50 == 0:
            ## GRAPH
            G_LOSS = abs(lossG_G.item())
            D_LOSS = abs(lossG_D.item())

            generator.append(G_LOSS)
            discriminator.append(D_LOSS)

            x = list(range(1, len(discriminator) + 1))
            plt.figure(1)
            plt.title('Training Loss')
            plt.plot(x, generator, 'g')
            plt.plot(x, discriminator, 'r')
            plt.xlabel('Iterations')
            plt.ylabel('Loss')
            plt.pause(1e-10)

            G_LOSS = np.mean(generator)
            D_LOSS = np.mean(discriminator)

            cgenerator.append(G_LOSS)
            cdiscriminator.append(D_LOSS)

            x = list(range(1, len(cdiscriminator) + 1))
            plt.figure(2)
            plt.title('Cumulative Training Loss')
            plt.plot(x, cgenerator, 'g')
            plt.plot(x, cdiscriminator, 'r')
            plt.xlabel('Iterations')
            plt.ylabel('Loss')
            plt.pause(1e-10)
            ## GRAPH
        ...
        if iter % opt.showTrainLossEvery == 0:  # print every 2000 mini-batches
            ...
            iter - opt.showTrainLossEvery + 1, iter, running_loss / (opt.showTrainLossEvery)))

            print('lossG_G for generator is %.5f respectively.' % (lossG_G.item()))

            # if opt.isGDL:
            #    print('loss for GDL loss is %f'%lossG_gdl.item())

            if opt.isAdLoss:
                print('loss_real is ', loss_real.item(), 'loss_fake is ', loss_fake.item(), 'outputD_real is',
                      outputD_real.data[0].item())
                print('loss for discriminator is %f' % lossD.item())
                print('lossG_D for discriminator is %f' % lossG_D.item())

            if opt.isWDist:
                print('lossG_D for discriminator is %f' % lossG_D.item())
                print('loss_real is ', torch.mean(D_real).item(), 'loss_fake is ', torch.mean(D_fake).item())
                print('loss for discriminator is %f' % Wasserstein_D.item(), ' D cost is %f' % D_cost)

            print(
                'cost time for iter [%d, %d] is %.2f' % (iter - opt.showTrainLossEvery + 1, iter, time.time() - start))
            print('************************************************')
            running_loss = 0.0
            start = time.time()
        ...
        if iter % opt.saveModelEvery == 0:  # save the model
            ...
            plt.figure(1)
            plt.savefig('trainstat.pdf', bbox_inches='tight')
            plt.figure(2)
            plt.savefig('avgtrainstat.pdf', bbox_inches='tight')
        if iter % opt.decLREvery == 0:
            ...
            print('LR:',opt.lr, 'LR_NETD:', opt.lr_netD)
        ...
        if iter % opt.showValPerformanceEvery == 0:  # test one subject
            ...
            print('lossG_G is %.2f' % (lossG_G.item()))

            if opt.isGDL:
                lossG_gdl = criterion_gdl(outputG, torch.unsqueeze(torch.squeeze(labels, 1), 1))
                print('loss for GDL loss is %.2f' % lossG_gdl.item())
        ...
        if iter % opt.showTestPerformanceEvery == 0:  # test one subject
            ...
            if opt.how2normalize == 6:
                ...
                maxPercentSPET, minPercentSPET = np.percentile(hpetnp, [99.5, 0])
                ...
                matSPET = (hpetnp - minPercentSPET) / (maxPercentSPET - minPercentSPET)
            ...
            if not opt.isMultiSource:
                ...
                if opt.how2normalize == 6:
                    ct_estimated = matOut * (maxPercentSPET - minPercentSPET) + minPercentSPET
                else:
                    ct_estimated = matOut
                ...
                print('gt: ', matSPET.dtype, ' shape: ', ct_estimated.shape)
                print('psnr = ', itspsnr)
